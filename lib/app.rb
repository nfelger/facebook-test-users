...
class FacebookTestUsers < Sinatra::Base
  ...
  get '/access_token' do
    token = /access_token=(.*)/.match(CurbFu.get("https://graph.facebook.com/oauth/access_token?client_id=#{params[:app_id]}&client_secret=#{params[:app_secret]}&grant_type=client_credentials").body)[1]
